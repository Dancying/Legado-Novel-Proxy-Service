...
class SixNineScraper(BaseWebScraper):

    def __init__(self):
        super().__init__()
        self.domain = "69shuba.com"
        self._search_url = "https://www.69shuba.com/modules/article/search.php"
        self._search_lock = FileLock(os.path.join(settings.TEMP_DIR, "sixnine_search.lock"), timeout=10)
        self._proxy_lock = FileLock(os.path.join(settings.TEMP_DIR, "sixnine_proxy.lock"), timeout=20)
        self._update_cookies_lock = FileLock(os.path.join(settings.TEMP_DIR, "sixnine_update_cookies.lock"), timeout=0)
        self._browser_cdp_version_file_path = os.path.join(settings.TEMP_DIR, "sixnine_browser_cdp_version")
    ...
    def search(self, keyword: str) -> str:
        try:
            with self._search_lock:
                ...
                if cookies:
                    data = {"searchkey": keyword.encode("gbk"), "submit": "Search"}
                    self.__set_headers()
                    ...
                    self._logger.warning("Sixnine cookies expired")
    ...
    def proxy(self, url: str) -> str:
        try:
            with self._proxy_lock:
                cookies = load_cookies(self.domain)
                if cookies:
                    whitelist = ["https://www.69shuba.com", "https://www.69yuedu.net", "https://69shuba.cx"]
                    self.__set_headers()
    ...
    def update_cookies(self) -> None:
        try:
            with self._update_cookies_lock:
                self._logger.info("Sixnine cookies update task starting")
                cookies = {}
                ...
                for i in range(0, 3):
                    is_verified = solve_cloudflare_turnstile(validation_url, ".container>div:last-of-type>div")
                    if is_verified:
                        cookies = get_domain_cookies(self.domain, True)
                        break
                write_json(get_edge_cdp_version(), self._browser_cdp_version_file_path)
                self._logger.debug("Sixnine CDP version info saved")
                save_cookies(cookies, self.domain)
                task_count = self._decrement_browser_task_count()
                if task_count == 0:
                    close_all_edge()
                time.sleep(5)
                self._logger.info("Sixnine cookies update task finished")
        except FileLockTimeoutError:
            self._logger.warning("Sixnine cookies update task is already running")
