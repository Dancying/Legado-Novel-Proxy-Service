...
class ShuXScraper(BaseWebScraper):

    def __init__(self):
        super().__init__()
        self._encoding = "UTF-8"
        self.domain = "69shux.co"
        self._search_url = "https://69shux.co/search"
        self._search_lock = FileLock(os.path.join(settings.TEMP_DIR, "shux_search.lock"), timeout=10)
        self._proxy_lock = FileLock(os.path.join(settings.TEMP_DIR, "shux_proxy.lock"), timeout=20)
        self._update_cookies_lock = FileLock(os.path.join(settings.TEMP_DIR, "shux_update_cookies.lock"), timeout=0)
        self._browser_cdp_version_file_path = os.path.join(settings.TEMP_DIR, "shux_browser_cdp_version")
    ...
    def search(self, keyword: str) -> str:
        try:
            with self._search_lock:
                ...
                if cookies:
                    data = {"searchkey": keyword, "submit": "Search"}
                    self.__set_headers()
                    ...
                    self._logger.warning("Shux cookies expired")
    ...
    def update_cookies(self) -> None:
        try:
            with self._update_cookies_lock:
                self._logger.info("Shux cookies update task starting")
                cookies = {}
                ...
                self._logger.debug("Shux CDP version info saved")
                save_cookies(cookies, self.domain)
                task_count = self._decrement_browser_task_count()
                if task_count == 0:
                    close_all_edge()
                time.sleep(3)
                self._logger.info("Shux cookies update task finished")
        except FileLockTimeoutError:
            self._logger.warning("Shux cookies update task is already running")
